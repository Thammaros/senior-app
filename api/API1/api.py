...
def Init():
    ...
    fer_interpreter = tf.lite.Interpreter(model_path="FER.tflite")
    ...
    global routes_idx
    routes_idx = tuple(x for x in mp_face_mesh.FACEMESH_FACE_OVAL)
    ...
    ser_interpreter = tf.lite.Interpreter(model_path="SER.tflite")
...
def SER_inference(input_video_file):
...
def inference(input_video_file):
    fer_output = FER_inference(input_video_file)
    ser_output = SER_inference(input_video_file)
    ...
    for idx,data in enumerate((fer_output,ser_output,weighted_output)):
        ...
        for idy,p in enumerate(graph_info):
            ...
            plt.text(x+width/2,
                     ...
                     ha='center',
                     weight='bold')
        plt.savefig(f'temp/out{idx}.jpeg')
        #line plot
        plt.figure(figsize=(15,10),dpi=100)
        plt.title(title_list[idx])
        ...
        plt.savefig(f'temp/out{idx+3}.jpeg')
...
async def create_upload_file(file: UploadFile):
    ...
    inference(f"temp/{file.filename}")
    result = []
    for i in range(6):
        img = Image.open(rf'temp/out{i}.jpeg')     
        buffered = BytesIO()
